...
\only<1-4>{
Modèle de Lifshitz : champ $\phi(x)$ à $n$ composantes, anisotrope. \\
Phase de Lifshitz : $\phi(x)$ périodique dans $m$ directions de l'espace de dimension $d$.
}
...
\[H[\phi(x_i)] \rightarrow \tilde{H}[\tilde{\phi}(x_b)]  \rightarrow \left.\begin{aligned}
        x' = x/S \\
        \phi' = S^\Delta \phi
       \end{aligned}
 \right\}
...
\centering
Représentation schématique des flots dans l'espace des couplages.
\[
H[\phi] = \sum_{i, \alpha} \kappa_\alpha O_\alpha[\phi(x_i), \nabla \phi(x_i), ...] \rightarrow H_S[\phi'] = \sum_{i', \alpha} \kappa_\alpha' O_\alpha[\phi'(x_i'), ...]
\]
}
